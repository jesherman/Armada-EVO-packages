...
- `patches/0520-arm64-dts-qcom-sm8550-add-a-pcie-suspend-opp.patch`
  source: armada
  upstream: not submitted
  notes: Companion DT for 0513. `opp-hz` is synthetic and `opp-level` is omitted so the OPP can never be selected for a trained link by either match in `qcom_pcie_icc_opp_update()`.
- `patches/0521-arm64-dts-qcom-sm8550-add-pcie1-suspend-opp.patch`
  source: Armada:EVO
  upstream: local
  notes: Adds the same 1000 kBps s2idle DDR/LLCC wake-contract floor to PCIe1, which the AYANEO Pocket common DTS enables for its Renesas uPD720201 USB host. Linux 7.2 already supplies the D3cold, PHY shutdown, and generic PCI power-control paths; this patch only fixes the missing PCIe1 suspend OPP.
